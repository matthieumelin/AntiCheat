...
public class LocationUtils {
    ...
    public static boolean isInLiquid(final Player player) {
        ...
        for (double x = -expand; x <= expand; x += expand) {
            for (double z = -expand; z <= expand; z += expand) {
                if (Objects.requireNonNull(getBlockAsync(location.clone().add(x, -0.5001, z))).isLiquid()) {
                    return true;
                }
            }
        }
    ...
    public static boolean isCollidingWithWeb(final Player player) {
        final Location location = player.getLocation();
        final int x = MathUtils.floor(location.getX());
        final int y = MathUtils.floor(location.getY());
        final int z = MathUtils.floor(location.getZ());
